...
function getApplicationClassName(folder) {
  ...
  if (files.length === 0) {
    ...
    const packages = files
      ...
      .map((file) =>
        file.match(
          /class\s+(\w+)\s*:\s*Application\(\)\s*,\s*ReactApplication/,
        ),
      )
      .filter((match) => match);
...
function updateManifestFile(manifestPath, applicationClassName) {
  ...
  var manifestXmlDoc = new DOMParser().parseFromString(
  ...
  for (i = 0; i < attrApplicationLength; i++) {
    ...
    if (attrNodeName.search("android:name") >= 0) {
      ...
      if (attrNodeValue === "." + applicationClassName) {
        ...
        attrApplication[0].removeAttribute(
          attrApplication[0].attributes[i].nodeName,
        );
        attrApplication[0].setAttribute(
          "android:name ",
...
function checkDatamiDomain(xmlDoc) {
  ...
  for (var i = 0; i < nodeNetworkConfig.length; i++) {
    if (nodeNetworkConfig[i].nodeType == 1) {
      ...
      if (
        null != domainName &&
        domainName.localeCompare("cloudmi.datami.com") == 0
      ) {
        available = true;
        break;
      }
...
function projectConfigAndroid(folder) {
  ...
  const applicationClassDetails = getApplicationClassName(
    sourceDir + "/src/main",
  );
  ...
  if (applicationClassName == null) {
    ...
    console.log(
      "Application class not available. Please use sample ApplicaitionClass.txt to create one and update AndroidManifest.xml with application name.",
    );
      ...
      if (isPackageExist < 0) {
        if (fileExtension === ".java") {
          ...
          if (intMainPackageIndex > 0) {
            // add package name
            var appfileNew = insert(
              appFile,
              intMainPackageIndex + 22,
              smiPackageName,
            );
            // add import
            ...
            if (intSuperIndex > 0) {
              appfileNew = insert(
                appfileNew,
                intSuperIndex + 17,
                stringParse + initSponsoredDataAPI,
              );
            } else {
          ...
          if (intMainPackageIndex > 0) {
            ...
            intMainPackageIndexFor073 > 0 ||
            intMainPackageIndexAbove073 > 0
          ) {
            // add package name
            var appfileNew = "";
            if (intMainPackageIndexFor073 > 0) {
              appfileNew = insert(
                appFile,
                intMainPackageIndexFor073,
                smiPackageNameFor73,
              );
            } else if (intMainPackageIndexAbove073 > 0) {
              appfileNew = insert(
                appFile,
                intMainPackageIndexAbove073 + searchTextAbove073.length,
                smiPackageNameAbove74,
              );
            }
            ...
            if (implementsIndex > 0) {
              appfileNew = insert(
                appfileNew,
                implementsIndex + searchText.length,
                stateChangeListnerStr,
              );
            }
